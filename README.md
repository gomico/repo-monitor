...
每行右侧的 7 格是近 7 天热力：**每格 = 该仓当日的字数变化量**，一行 7 格连起来才是近 7 天的趋势。配色档位（浅蓝 → 深蓝 → 琥珀 → 橙红 → 红 → 深红，共 10 档）由 `monitor.REPORT_HEAT_TIERS` 单独定义，和左侧栏的 5 个改动档位分组、以及侧栏「近 7 天变化量排行」（7 天合计）互不影响；悬停可见日期、字数与所在档位，报表底部的图例列出各档区间。
